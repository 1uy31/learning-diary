...
def upgrade():
    ...
    op.create_foreign_key(None, 'diary', 'category', ['category_id'], ['id'], ondelete='RESTRICT')
    op.drop_column('diary', 'category')
    op.drop_column('diary', 'note')
    op.add_column('note', sa.Column('diary_id', sa.Integer(), nullable=True))
    op.create_foreign_key(None, 'note', 'diary', ['diary_id'], ['id'], ondelete='CASCADE')
...
def downgrade():
    ...
    op.add_column('diary', sa.Column('note', sa.INTEGER(), autoincrement=False, nullable=True))
    op.add_column('diary', sa.Column('category', sa.INTEGER(), autoincrement=False, nullable=True))
    op.drop_constraint(None, 'diary', type_='foreignkey')
    op.create_foreign_key('diary_category_fkey', 'diary', 'category', ['category'], ['id'], ondelete='RESTRICT')
    op.create_foreign_key('diary_note_fkey', 'diary', 'note', ['note'], ['id'], ondelete='CASCADE')
    op.drop_constraint(None, 'diary', type_='unique')
    op.create_unique_constraint('diary_category_topic_key', 'diary', ['category', 'topic'])
